...
def visualize_silhouette_plot(data, optimal_k):
    ...
    plt.text(avg_score + 0.02, plt.ylim()[0] + 0.05 * (plt.ylim()[1] - plt.ylim()[0]), 
             f"Average: {avg_score:.3f}", color="red")
...
def calculate_shap_values(data, cluster_labels, feature_names):
    ...
    if isinstance(shap_values, list) and len(shap_values) == len(unique_clusters):
        ...
        print("Warning: SHAP values structure doesn't match the expected format for per-cluster plots.")
...
def visualize_clusters_pca(data, cluster_labels, feature_names):
    ...
    # Improve legend
    plt.legend(title='Cluster', title_fontsize=14, fontsize=12, bbox_to_anchor=(1.05, 1), loc='upper left')
    ...
    for cluster in np.unique(cluster_labels):
        ...
        plt.scatter(centroid['PC1'], centroid['PC2'], s=200, marker='X', color='black', edgecolor='white', linewidth=2, label=f'Centroid {cluster}' if cluster == 0 else "")
        plt.annotate(f'Cluster {cluster}', (centroid['PC1'], centroid['PC2']), fontsize=14, fontweight='bold', ha='center', va='bottom', xytext=(0, 10), textcoords='offset points')
    ...
    for i, feature in enumerate(feature_names):
        plt.arrow(0, 0, loadings[i, 0], loadings[i, 1], head_width=0.05, head_length=0.05, fc='blue', ec='blue', alpha=0.5)
        plt.text(loadings[i, 0] * 1.15, loadings[i, 1] * 1.15, feature, color='blue', ha='center', va='center', fontsize=12)
...
def main():
    ...
    print("\nCluster Summary:")
    print(cluster_stats[['count', 'imd_score_normalized_mean', 'NO2_normalized_mean', 
                         'PM2.5_normalized_mean', 'respiratory_health_index_mean', 
                         'vulnerability_index_mean']])
    
    # Save cluster assignments
    df_for_clustering.loc[:, 'cluster'] = cluster_labels
    df_for_clustering.to_csv('outputs/advanced_clustering/lad_with_advanced_clusters.csv', index=False)
