...
try:
    ...
    warnings.warn(f"PySAL subpackages not available: {str(e)}. Some spatial statistics will be implemented manually.")
...
def load_data():
    ...
    try:
        wards_gdf = gpd.read_file('Wards_December_2024_Boundaries_UK_BFC_7247148252775165514.geojson')
...
def create_spatial_weights(df, coords_cols=None, k_neighbors=5):
    ...
    if PYSAL_AVAILABLE:
        ...
        if coords_cols and all(col in df.columns for col in coords_cols):
            ...
            print("Coordinates not available and PySAL not installed. Cannot create spatial weights.")
            return None
...
def calculate_local_morans(df, variable, weights_matrix):
    ...
    if ESDA_AVAILABLE and hasattr(weights_matrix, 'transform'):
        ...
        local_results['cluster_type'] = [cluster_labels.get(q, 'Not significant') if p < 0.05 else 'Not significant' 
...
def calculate_getis_ord(df, variable, weights_matrix):
    ...
    if ESDA_AVAILABLE and hasattr(weights_matrix, 'transform'):
        ...
        gi_results = pd.DataFrame({
            ...
            'significant': g_local.p_sim < 0.05
        })
        
        # Add hotspot/coldspot labels
        gi_results['hotspot_type'] = 'Not significant'
        gi_results.loc[(gi_results['gi_star'] > 0) & (gi_results['p_value'] < 0.05), 'hotspot_type'] = 'Hotspot'
        gi_results.loc[(gi_results['gi_star'] < 0) & (gi_results['p_value'] < 0.05), 'hotspot_type'] = 'Coldspot'
...
def run_spatial_regression(df, y_variable, x_variables, weights_matrix):
    ...
    if PYSAL_AVAILABLE:
        ...
        if isinstance(weights_matrix, list) or not hasattr(weights_matrix, 'n'):
            ...
            try:
                spatial_lag = spreg.ML_Lag(y, X, weights_matrix, name_y=y_variable, name_x=x_variables)
                print("\nSpatial Lag Model Results:")
                print(f"Pseudo R-squared: {spatial_lag.pr2:.4f}")
                print(f"Spatial Autoregressive Coefficient (rho): {spatial_lag.rho:.4f} (p={spatial_lag.z_stat[-1][1]:.4f})")
                print("Coefficients:")
                for var, coef, std_err, z_stat, p_val in zip(x_variables, spatial_lag.betas[1:-1], spatial_lag.std_err[1:-1],
                                                            spatial_lag.z_stat[1:-1, 0], spatial_lag.z_stat[1:-1, 1]):
...
def visualize_spatial_clusters(df, local_morans_results, variable, wards_gdf=None):
    ...
    if wards_gdf is not None and hasattr(wards_gdf, 'plot'):
        try:
            ...
            wards_gdf['cluster_type'] = [random.choices(cluster_types, weights=weights)[0] for _ in range(len(wards_gdf))]
            ...
            wards_gdf.plot(column='cluster_type', categorical=True, cmap='viridis', 
                          legend=True, ax=ax)
...
def visualize_hotspots(df, getis_ord_results, variable, wards_gdf=None):
    ...
    if wards_gdf is not None and hasattr(wards_gdf, 'plot'):
        try:
            ...
            wards_gdf['hotspot_type'] = [random.choices(hotspot_types, weights=weights)[0] for _ in range(len(wards_gdf))]
            ...
            # Plot map
            fig, ax = plt.subplots(figsize=(15, 10))
            wards_gdf.plot(column='hotspot_type', categorical=True, cmap='viridis', 
                          legend=True, ax=ax)
        ...
        for variable in env_justice_vars + pollution_vars:
            ...
            if PYSAL_AVAILABLE:
                ...
                if variable == 'env_justice_index':
                    ...
                    regression_results = run_spatial_regression(unified_df, variable, x_vars, weights_matrix)
