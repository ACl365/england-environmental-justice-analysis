...
def identify_deprivation_domains(unified_df):
    ...
    if not available_domains:
        ...
        potential_domains = [
            ...
            if "deprivation" in col.lower() or "score" in col.lower()
        ]
...
def analyze_domain_correlations(unified_df, domain_vars, domain_names):
    ...
    corr_matrix_labeled = corr_matrix_labeled.rename(
        index=dict(zip(domain_vars, domain_names)), columns=dict(zip(domain_vars, domain_names))
    )
    ...
    for d_var, d_name in zip(domain_vars, domain_names):
        ...
        if not d_var.endswith("_normalized"):
            ...
            if "rank" in d_var and not "where_1_is_most_deprived" in d_var:
                ...
                domain_normalized = 1 - (unified_df[d_var] - unified_df[d_var].min()) / (
                    unified_df[d_var].max() - unified_df[d_var].min()
                )
            else:
                # For scores, higher is worse, so normalize directly
                domain_normalized = (unified_df[d_var] - unified_df[d_var].min()) / (
                    unified_df[d_var].max() - unified_df[d_var].min()
                )
        ...
        for p_var in pollution_vars:
            ...
            df_with_indices[index_name] = (
                df_with_indices[norm_var_name] + df_with_indices[p_var]
            ) / 2
            ...
            df_with_indices[index_name] = (
                100 * (df_with_indices[index_name] - min_val) / (max_val - min_val)
            )

            print(
                f"Created {index_name} (mean: {df_with_indices[index_name].mean():.2f}, std: {df_with_indices[index_name].std():.2f})"
            )
...
def identify_domain_hotspots(df_with_indices, domain_names):
    ...
    if hotspot_summary:
        ...
        hotspot_summary_df.to_csv(
            "outputs/domain_deprivation/hotspot_summary_by_lad.csv", index=False
        )

        # Create pivot table for visualization
        pivot_table = hotspot_summary_df.pivot_table(
            index="LAD", columns=["Domain", "Pollution"], values="Count", fill_value=0
        )
...
def create_radar_charts(df_with_indices, domain_vars, domain_names):
    ...
    lad_aggregated = (
        df_with_indices.groupby("lad_code")
        .agg({"lad_name": "first", **{var: "mean" for var in domain_vars}})
        .reset_index()
    )
    ...
    for var in domain_vars:
        ...
        if "rank" in var and not "where_1_is_most_deprived" in var:
            # For ranks, higher is better, so invert
            domain_normalized[var] = 1 - (lad_aggregated[var] - lad_aggregated[var].min()) / (
                lad_aggregated[var].max() - lad_aggregated[var].min()
            )
        else:
            # For scores, higher is worse, so normalize directly
            domain_normalized[var] = (lad_aggregated[var] - lad_aggregated[var].min()) / (
                lad_aggregated[var].max() - lad_aggregated[var].min()
            )
    ...
    lad_aggregated["overall_deprivation"] = np.mean(
        [domain_normalized[var] for var in domain_vars], axis=0
    )
    ...
    for idx, row in top_deprived.iterrows():
        ...
        fig.add_trace(
            go.Scatterpolar(r=values, theta=domain_names, fill="toself", name=row["lad_name"])
        )
    ...
    try:
        ...
        lad_aggregated["NO2_level"] = pd.cut(
            no2_values, bins=bin_edges, labels=["Low", "Medium", "High"], include_lowest=True
        )

    # Calculate average domain profiles by pollution level
    pollution_profiles = (
        lad_aggregated.groupby("NO2_level", observed=False)
        .agg({**{f"{var}_normalized": "mean" for var in domain_vars}})
        .reset_index()
    )
    ...
    for idx, row in pollution_profiles.iterrows():
        ...
        fig.add_trace(
            go.Scatterpolar(
                r=values, theta=domain_names, fill="toself", name=f"NO2: {row['NO2_level']}"
            )
        )
...
def analyze_domain_importance(unified_df, domain_vars, domain_names):
    ...
    for var in domain_vars:
        ...
        if "rank" in var and not "where_1_is_most_deprived" in var:
            # For ranks, higher is better, so invert
            domain_normalized[var] = 1 - (analysis_data[var] - analysis_data[var].min()) / (
                analysis_data[var].max() - analysis_data[var].min()
            )
        else:
            # For scores, higher is worse, so normalize directly
            domain_normalized[var] = (analysis_data[var] - analysis_data[var].min()) / (
                analysis_data[var].max() - analysis_data[var].min()
            )
    ...
    for p_var in pollution_vars:
        ...
        importance_df = pd.DataFrame(
            {"Domain": domain_names, "Importance": importances}
    ...
    plt.figure(figsize=(12, 8))
    importance_pivot = all_importance.pivot(
        index="Domain", columns="Pollutant", values="Importance"
    )
