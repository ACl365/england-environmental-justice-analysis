...
def create_pollution_index_pca(merged_df):
    ...
    loadings_df = pd.DataFrame(
        loadings, columns=[f"PC{i+1}" for i in range(len(pollutant_vars))], index=pollutant_vars
    )
    ...
    merged_df_with_pca["pollution_index_pca_normalized"] = (
        100 * (merged_df_with_pca["pollution_index_pca"] - min_val) / (max_val - min_val)
    )
    ...
    merged_df_with_pca.to_csv(
        "outputs/pollutant_interactions/lad_with_pollution_index.csv", index=False
    )
...
def analyze_interaction_terms(merged_df):
    ...
    pollutants_scaled_df = pd.DataFrame(
        pollutants_scaled, columns=pollutant_vars, index=analysis_data.index
    )
    ...
    for i, p1 in enumerate(pollutant_vars):
        for p2 in pollutant_vars[i + 1 :]:
            ...
            pollutants_scaled_df[interaction_name] = (
                pollutants_scaled_df[p1] * pollutants_scaled_df[p2]
            )
            interaction_pairs.append((p1, p2, interaction_name))

    # Combine with health variable
    analysis_data_with_interactions = pd.concat(
        [pollutants_scaled_df, analysis_data[health_var]], axis=1
    )
    ...
    for p1, p2, interaction_name in interaction_pairs:
        ...
        if p_value < 0.1:  # Using 0.1 as threshold for potential significance
            significant_interactions.append(
                (p1, p2, interaction_name, model.params[interaction_name], p_value)
            )
    ...
    # Implement Gradient Boosting Regressor (GBR) model
    print("\nImplementing Gradient Boosting Regressor (GBR) model...")
    ...
    cv_scores = cross_val_score(gbr, X, y, cv=5)  # 5-fold cross-validation
    ...
    # Calculate SHAP values for GBR model
    print("\nCalculating SHAP values for GBR model...")
    ...
    shap.summary_plot(
        shap_values[:, :-1],
        features=X_no_const,
        feature_names=pollutants_scaled_df.columns,
        show=False,
    )
    plt.tight_layout()
    plt.savefig("outputs/pollutant_interactions/shap_summary_gbr.png", dpi=300)
...
def visualize_interaction(data, var1, var2, response_var):
    ...
    fig = go.Figure(
        data=[
            ...
            go.Scatter3d(
                ...
                z=data[response_var],
                mode="markers",
                marker=dict(size=5, color=data[response_var], colorscale="Inferno", opacity=0.8),
            ),
        ]
    )
    ...
    fig.update_layout(
        ...
        scene=dict(xaxis_title=var1, yaxis_title=var2, zaxis_title=response_var),
        width=900,
        height=700,
    )
...
def threshold_effect_analysis(merged_df):
    ...
    for i, pollutant in enumerate(pollutant_vars):
        ...
        sorted_data["rolling_health"] = (
            sorted_data[health_var].rolling(window=window_size, center=True).mean()
        )
        ...
        for threshold in thresholds:
            ...
            sorted_data["pollutant_above"] = sorted_data["above_threshold"] * (
                sorted_data[pollutant] - threshold
            )
        ...
        if best_threshold is not None:
            ...
            pred_data["pollutant_above"] = pred_data["above_threshold"] * (
                pred_data[pollutant] - best_threshold
            )
            ...
            # Plot prediction
            ax.plot(
                pred_x, pred_y, "g-", linewidth=2, label=f"Threshold model (t={best_threshold:.2f})"
            )
            ...
            print(
                f"Total effect after threshold: {best_model.params[pollutant] + best_model.params['pollutant_above']:.4f}"
            )
    ...
    for pollutant in pollutant_vars:
        ...
        threshold = np.percentile(
            sorted_data[pollutant], 60
        )  # Example threshold at 60th percentile
        ...
        # Update layout
        fig.update_layout(
            width=900, height=600, legend=dict(yanchor="top", y=0.99, xanchor="left", x=0.01)
        )
