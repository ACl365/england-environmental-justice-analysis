...
between air pollution, socioeconomic deprivation, and respiratory health outcomes in the UK.
"""
...
def load_data():
    ...
    if os.path.exists(os.path.join(OUTPUT_DATA_PATH, "lad_clustered.csv")):
        ...
        high_vulnerability = pd.read_csv(os.path.join(OUTPUT_DATA_PATH, "high_vulnerability_areas.csv"))
        ...
        lad_data = lsoa_data.groupby('lad_code').agg({
            'lad_name': 'first',
        ...
        high_vulnerability = lad_data.sort_values('respiratory_health_index', ascending=False).head(20)
...
def main():
    ...
    # Header
    st.markdown('<div class="main-header">Environmental Justice and Health Inequalities in the UK</div>', 
                unsafe_allow_html=True)
    
    st.markdown("""
    <div class="highlight">
    This dashboard explores the relationship between air pollution, socioeconomic deprivation, 
    and respiratory health outcomes across the UK. It identifies areas of environmental injustice 
    ...
    page = st.sidebar.radio(
        "Select a page:",
        ["Overview", "Environmental Justice Analysis", "Health Outcomes", 
         "Vulnerability Index", "Cluster Analysis", "Intervention Priorities"]
    )
    ...
    if 'region' in lad_data.columns:
        selected_region = st.sidebar.multiselect(
            "Select Region(s):",
            options=sorted(lad_data['region'].unique()),
            default=[]
        )
    else:
        selected_region = []
    ...
    pollution_metric = st.sidebar.selectbox(
        "Pollution Metric:",
        ["NO2", "PM2.5", "PM10", "O3"]
    )
    ...
    filtered_lad_data = filtered_lad_data[filtered_lad_data[pollution_metric] >= pollution_threshold]
...
def display_overview(lad_data, lsoa_data):
    ...
    with col1:
        ...
        """.format(len(lad_data)), unsafe_allow_html=True)
    ...
    with col2:
        ...
        """.format(len(lsoa_data)), unsafe_allow_html=True)
    ...
    with col3:
        ...
        """.format(lad_data['NO2'].mean()), unsafe_allow_html=True)
    ...
    with col4:
        ...
        """.format(lad_data['PM2.5'].mean()), unsafe_allow_html=True)
    ...
    # Map of UK with pollution levels
    st.markdown('<div class="section-header">Air Pollution Distribution</div>', unsafe_allow_html=True)
    
    # Create a placeholder for the map (would be replaced with actual map in a full implementation)
    if 'latitude' in lad_data.columns and 'longitude' in lad_data.columns:
        fig = px.scatter_mapbox(
            ...
            size="PM2.5",
            hover_name="lad_name",
            hover_data=["NO2", "PM2.5", "PM10", "O3"],
            color_continuous_scale="Viridis",
            zoom=5,
            mapbox_style="carto-positron",
            title="Air Pollution Levels Across the UK"
        )
        st.plotly_chart(fig, use_container_width=True)
    else:
        st.info("Geographic coordinates not available for mapping. Please run the full analysis script to generate this visualization.")
        
        # Display a sample distribution instead
        fig = px.histogram(
            lad_data,
            x="NO2",
            nbins=30,
            title="Distribution of NO₂ Levels Across Local Authorities"
        )
        st.plotly_chart(fig, use_container_width=True)

def display_environmental_justice(lad_data, lsoa_data, high_injustice):
    """Display the environmental justice analysis page."""
    st.markdown('<div class="sub-header">Environmental Justice Analysis</div>', unsafe_allow_html=True)
    ...
    st.markdown('<div class="section-header">Relationship Between Air Pollution and Deprivation</div>', 
                unsafe_allow_html=True)
    
    pollution_metric = st.selectbox(
        "Select Pollution Metric:",
        ["NO2", "PM2.5", "PM10", "O3"],
        key="pollution_scatter"
    )
    ...
    st.markdown('<div class="section-header">Environmental Justice Index Distribution</div>', 
                unsafe_allow_html=True)
    ...
    st.markdown('<div class="section-header">Areas with Highest Environmental Injustice</div>', 
                unsafe_allow_html=True)
...
def display_health_outcomes(lad_data):
    ...
    st.markdown('<div class="section-header">Relationship Between Air Pollution and Health Outcomes</div>', 
                unsafe_allow_html=True)
    ...
    with col1:
        pollution_metric = st.selectbox(
            "Select Pollution Metric:",
            ["NO2", "PM2.5", "PM10", "O3"],
            key="health_pollution"
        )
    
    with col2:
        health_metric = st.selectbox(
            "Select Health Metric:",
            ["respiratory_health_index", "chronic_conditions_normalized", 
    ...
    st.markdown('<div class="section-header">Double Disadvantage Analysis</div>', 
                unsafe_allow_html=True)
...
def display_vulnerability_index(lad_data, high_vulnerability):
    ...
    if 'vulnerability_index' not in lad_data.columns:
        st.warning("Vulnerability Index not found in the data. Please run the full analysis script to generate this index.")
        ...
        lad_data['vulnerability_index'] = 100 * (lad_data['vulnerability_index'] - min_val) / (max_val - min_val)
    ...
    st.markdown('<div class="section-header">Vulnerability Index Distribution</div>', 
                unsafe_allow_html=True)
    ...
    st.markdown('<div class="section-header">Areas with Highest Vulnerability</div>', 
                unsafe_allow_html=True)
    
    # Determine which columns to display
    display_cols = ['lad_name', 'vulnerability_index']
    for col in ['imd_score_normalized', 'NO2_normalized', 'PM2.5_normalized', 'respiratory_health_index']:
    ...
    st.markdown('<div class="section-header">Vulnerability Components Analysis</div>', 
                unsafe_allow_html=True)
    
    # Check if we have the necessary columns
    component_cols = ['imd_score_normalized', 'NO2_normalized', 'PM2.5_normalized', 'respiratory_health_index']
    ...
    if not missing_cols:
        ...
        for i, row in top_areas.iterrows():
            fig.add_trace(go.Scatterpolar(
                r=[row['imd_score_normalized'], row['NO2_normalized'], 
                   row['PM2.5_normalized'], row['respiratory_health_index']],
...
def display_cluster_analysis(lad_data):
    ...
    if 'cluster' not in lad_data.columns:
        st.warning("Cluster assignments not found in the data. Please run the full analysis script to generate clusters.")
    ...
    cluster_summary = lad_data.groupby('cluster').agg({
        'lad_name': 'count',
    }).rename(columns={'lad_name': 'count'})
    
    # Add mean values for key variables if they exist
    for col in ['imd_score_normalized', 'NO2_normalized', 'PM2.5_normalized', 
    ...
    st.markdown('<div class="section-header">Cluster Visualization</div>', 
                unsafe_allow_html=True)
    
    # Check if we have the necessary columns for visualization
    viz_cols = ['imd_score_normalized', 'NO2_normalized', 'PM2.5_normalized', 'respiratory_health_index']
